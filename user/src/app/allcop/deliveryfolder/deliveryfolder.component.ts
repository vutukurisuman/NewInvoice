...
@Component({
  selector: 'app-deliveryfolder',
  templateUrl: './deliveryfolder.component.html',
  styleUrls: ['./deliveryfolder.component.scss']
})
export class DeliveryfolderComponent implements OnInit {
  ...
  if(foldertype=="Delivery Challan"){
    this.http.getdeliveryfolderuser(data).subscribe((res)=>{
      this.clients=res
      for (var i = 0; i < this.clients.length; i++) {        
        this.clients[i]['sequencyId'] = i;
    }
